        ...
        <div className="md:col-span-8 space-y-8">
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-2">
                ...
                <div className="h-3 w-full rounded-full bg-slate-100 overflow-hidden border border-slate-200/40">
                  <div className="h-full rounded-full bg-teal-600 transition-all duration-500" style={{ width: '45%' }} />
                </div>
                <p className="text-[11px] text-slate-500">Đã học xong 6/12 bài giảng • Ghi chú: 8 bản ghi</p>
              </div>
          ...
          <div className="rounded-3xl border border-slate-200/80 bg-white p-6 shadow-sm space-y-6">
            ...
            <div className="grid gap-4 grid-cols-2 sm:grid-cols-3">
              {[
                { name: 'Người khởi đầu', desc: 'Học bài học đầu tiên', icon: Zap, color: 'bg-teal-50 text-teal-600 border border-teal-200' },
                { name: 'Chiến binh Streak', desc: 'Học liên tiếp 5 ngày', icon: Flame, color: 'bg-orange-50 text-orange-600 border border-orange-200' },
                { name: 'Cao thủ Từ vựng', desc: 'Thuộc hơn 100 từ vựng', icon: Star, color: 'bg-yellow-50 text-yellow-700 border border-yellow-200' }
              ].map((badge, idx) => (
                <div key={idx} className="flex flex-col items-center text-center p-4 bg-slate-50 rounded-2xl border border-slate-100 space-y-2">
